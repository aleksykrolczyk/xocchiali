...
uint16_t xocchiali_handle_table[GATT_DB_LAST_INDEX];
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        case ESP_GATTS_REG_EVT:{
            ...
            adv_config_done |= scan_rsp_config_flag;
#endif
            esp_err_t create_attr_ret = esp_ble_gatts_create_attr_tab(GATT_DB, gatts_if, GATT_DB_LAST_INDEX, SVC_INST_ID);
            if (create_attr_ret){
...
//            ESP_LOGI(GATTS_TAG, "GATT_READ_EVT, conn_id %d, trans_id %d, handle %d\n", param->read.conn_id, param->read.trans_id, param->read.handle);
            ...
            uint16_t value = touchpad_value(PLAY_BUTTON);
//            ESP_LOGI(GATTS_TAG, "READ PLAY VALUE: %d", value);

            rsp.attr_value.handle = param->read.handle;
            rsp.attr_value.len = 2;
            rsp.attr_value.value[0] = value & 0xFF;
            rsp.attr_value.value[1] = value >> 8;
        ...
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                ...
                if (param->write.handle == xocchiali_handle_table[IDX_CHARACTERISTIC_VALUE_LED] && param->write.len == sizeof(led_value)) {
                    ESP_LOGI(GATTS_TAG, "WRITE EVENT, VALUE: %s", param->write.value);
                    set_led_state(BLINK_GPIO, param->write.value[0]);
...
//                        esp_ble_gatts_send_indicate(gatts_if, param->write.conn_id, xocchiali_handle_table[IDX_CHARACTERISTIC_VALUE_LED],
...
//            example_exec_write_event_env(&prepare_write_env, param);
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            ...
            else if (param->add_attr_tab.num_handle != GATT_DB_LAST_INDEX){
                ESP_LOGE(GATTS_TAG, "create attribute table abnormally, num_handle (%d) \
                        doesn't equal to HRS_IDX_NB(%d)", param->add_attr_tab.num_handle, GATT_DB_LAST_INDEX);
            }
            else {
                ESP_LOGI(GATTS_TAG, "create attribute table successfully, the number handle = %d\n",param->add_attr_tab.num_handle);
                memcpy(xocchiali_handle_table, param->add_attr_tab.handles, sizeof(xocchiali_handle_table));
                esp_ble_gatts_start_service(xocchiali_handle_table[IDX_SVC]);
            }
            break;
        }
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    if (gatts_if == GATTS_PROFILE.gatts_if && GATTS_PROFILE.gatts_cb) {
...
void app_main(void)
{
    ...
    ESP_LOGI(GATTS_TAG, "Configuring play button");
    configure_play_button(PLAY_BUTTON);

    ESP_LOGI(GATTS_TAG, "Configuring LED light");
